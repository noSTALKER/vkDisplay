...
class InputAttachmentApplication : public vkDisplay::Application
{
public:
	InputAttachmentApplication() {}
...
private:
	...
	vk::Sampler mSampler;
	vk::DescriptorSetLayout mSamplerSetLayout;
	vk::DescriptorSetLayout mInputSetLayout;
...
vk::Result
InputAttachmentApplication::createRenderpass()
{
	vk::Result result;
	...
	vk::AttachmentReference inputAttachmentRef(2, vk::ImageLayout::eShaderReadOnlyOptimal);
	...
	//second subpass is only a fullscreen subpass which reads the input attachment populated by the first pass
	...
	fullscreenPass.inputAttachmentCount = 1;
	fullscreenPass.pInputAttachments = &inputAttachmentRef;
...
vk::Result
InputAttachmentApplication::createResources()
{
	vk::Result result;
	//buffer data
	struct BufferData {
...
vk::Result
InputAttachmentApplication::createFramebuffers()
{
	vk::Result result;
...
vk::Result
InputAttachmentApplication::createPipeline()
{
	vk::Result result;
	...
	//pipeline layout
	vk::PipelineLayoutCreateInfo pipelineLayoutCreateInfo({}, 1, &mSamplerSetLayout, 0, nullptr);
	...
	//
	vk::DescriptorSetLayoutBinding inputSetLayoutBinding{ 0, vk::DescriptorType::eInputAttachment, 1, vk::ShaderStageFlagBits::eFragment, nullptr };
	vk::DescriptorSetLayoutCreateInfo inputSetLayoutCreateInfo({}, 1, &inputSetLayoutBinding);
	std::tie(result, mInputSetLayout) = mDevice.createDescriptorSetLayout(inputSetLayoutCreateInfo);

	//pipeline layout
	vk::PipelineLayoutCreateInfo fullscreenPipelineLayoutCreateInfo({}, 1, &mInputSetLayout, 0, nullptr);
...
vk::Result
InputAttachmentApplication::createCommandBuffers()
{
	vk::Result result;
	...
	vk::DescriptorPoolSize samplerPoolSize(vk::DescriptorType::eCombinedImageSampler, 1);
	vk::DescriptorPoolCreateInfo samplerDescriptorPoolCreateInfo({}, 1, 1, &samplerPoolSize);

	vk::DescriptorPool samplerDescriptorPool;
	std::tie(result, samplerDescriptorPool) = mDevice.createDescriptorPool(samplerDescriptorPoolCreateInfo);
	vk::DescriptorSetAllocateInfo samplerAllocateInfo(samplerDescriptorPool, 1, &mSamplerSetLayout);
	std::vector<vk::DescriptorSet> sets;
	std::tie(result, sets) = mDevice.allocateDescriptorSets(samplerAllocateInfo);
	...
	vk::DescriptorPoolSize inputPoolSize(vk::DescriptorType::eInputAttachment, 1);
	vk::DescriptorPoolCreateInfo inputDescriptorPoolCreateInfo({}, 1, 1, &inputPoolSize);

	vk::DescriptorPool inputDescriptorPool;
	std::tie(result, inputDescriptorPool) = mDevice.createDescriptorPool(inputDescriptorPoolCreateInfo);
	vk::DescriptorSetAllocateInfo inputAllocateInfo(inputDescriptorPool, 1, &mInputSetLayout);

	vk::DescriptorSetAllocateInfo fullscreenAllocateInfo(inputDescriptorPool, 1, &mInputSetLayout);
	...
	vk::DescriptorImageInfo fullscreenImageInfo(vk::Sampler(), mRenderImageView, vk::ImageLayout::eShaderReadOnlyOptimal);
	vk::WriteDescriptorSet fullscreenWriteDescriptorSet(fullscreenSets[0], 0, 0, 1, vk::DescriptorType::eInputAttachment, &fullscreenImageInfo);
	...
	for (std::size_t i = 0; i < mSwapchainImageViews.size(); ++i) {
		...
		//no need to bind the vertex and index buffer again as we use same vertex data in both subpasses
...
void
InputAttachmentApplication::render(double frameTime, double totalTime)
{
	vk::Result result;
